...
def test_fixtures_build_load_and_determinism(tmp_path):
    ...
    labels, channels = fixtures.load_fixture(tmp_path / "a", a["fixtures"][0])  # verifies sha256
...
def test_run_enumerates_and_times_all_functions(tmp_path):
    ...
    expected = set(bulk.get_core_measurements()) | set(bulk.get_correlation_measurements())
    assert {f.label for f in run.enumerate_functions() if "[legacy]" not in f.label} == expected
    ...
    assert statuses and all(s == "ok" for s in statuses)  # incl. texture (needs [0,1] norm)
...
def test_time_call_error_and_timeout():
    ...
    assert run.time_call(_F(lambda *a: time.sleep(1)), (), 0, 1, 0.05)["status"] == "timeout"


def _report(times):
    fix = [{"key": "k", "size": 128, "n_objects": 4, "seed": 0}]
    results = {fn: {"k": {"status": "ok", "min": s, "median": s}} for fn, s in times.items()}
    return {"meta": {"synth_version": "0", "reps": 1}, "fixtures": fix, "results": results}


def test_compare_classifies_and_renders():
    base = _report({"fast": 0.010, "slow": 0.010, "same": 0.010, "gone": 0.010, "err": 0.010})
    head = _report({"fast": 0.005, "slow": 0.020, "same": 0.0101, "fresh": 0.010, "err": 0.010})
    head["results"]["err"]["k"] = {"status": "error", "error": "x"}
    rows = {r["function"]: r for r in compare.compare(base, head)}
    assert rows["fast"]["status"] == "faster" and rows["fast"]["speedup"] == pytest.approx(2.0)
    ...
    md = compare.render_markdown(compare.compare(base, head), base["meta"], head["meta"])
