...
def _make_fused_upsample_mean(
    orig_shape: NDArray[numpy.integer],
    new_shape: NDArray[numpy.integer],
    ...
    direct interpolation. Labels are the contiguous ``1..N`` cp_measure guarantees (see
    :mod:`cp_measure._sanitize`), so operator/count row ``L`` is object ``L``.
    """
    ...
    # map_coordinates(mode='constant', cval=0) returns 0 for any source coordinate outside
    # [0, new-1]; the float-rounded scale can push the last row/col just past new-1
    ...
    objects = slice(1, max_label + 1)  # rows for labels 1..N; row 0 is background
...
def get_granularity(
    ...
    fused_mean = None
    if pixels.ndim == 2 and range_.size:
