...
- **Fidelity and 0–1 normalization**: To match CellProfiler, convert images to floating point in `[0, 1]` using the data type's full range (for example, `uint8 / 255` or `uint16 / 65535`). This is a data-type conversion, not per-image min–max normalization.
  - **Directly rescaled**: `Intensity_IntegratedIntensity`, `Intensity_MeanIntensity`, `Intensity_StdIntensity`, `Intensity_MinIntensity`, `Intensity_MaxIntensity`, their five `*IntensityEdge` variants, and the four quantile/MAD measurements are divided by the conversion factor. Every `RadialDistribution_ZernikeMagnitude_*` measurement scales the same way; `RadialDistribution_ZernikePhase_*` does not.
  - **Requantized**: all Haralick texture measurements (`AngularSecondMoment_*` through `InfoMeas2_*`) can change because texture first converts the image to discrete gray levels. Unnormalized floating-point input outside `[-1, 1]` is rejected by this conversion.
  - **Range/dtype-sensitive**: raw integer intermediates can overflow and alter all `RadialDistribution_FracAtD_*`, `RadialDistribution_MeanFrac_*`, and `RadialDistribution_RadialCV_*` measurements, as well as `Correlation_Costes_*` and the low-level `Correlation_Overlap`, `Correlation_K_1`, and `Correlation_K_2` measurements. Normalize before computing them.
  - **Unchanged under positive rescaling**: shape measurements (`sizeshape`, shape Zernikes, and Feret), `Granularity_*`, `Location_*`, `Intensity_MassDisplacement`, radial Zernike phases, `Correlation_Pearson`, `Correlation_Manders_*`, and `Correlation_RWC_*` are unaffected apart from floating-point roundoff. `Correlation_Slope`, `Correlation_K_1`, and `Correlation_K_2` are also unchanged when every channel uses the same conversion factor, but change when channel factors differ.
  - **Legacy quantiles**: for `LowerQuartileIntensity`, `MedianIntensity`, `UpperQuartileIntensity`, and `MADIntensity`, you additionally need `legacy=True` for CellProfiler fidelity — see below.
